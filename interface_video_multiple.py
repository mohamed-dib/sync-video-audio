...
class VideoImageSelecteur(QMainWindow):
    ...
    def __init__(self, video_path , index =0,on_close_callback=None ):
        super().__init__()
        self.on_close_callback = on_close_callback
        self.index = index
        ## Conversion automatique de la vidéo en 640×360, H.264, 30 fps
    ...
    #l'exportation est différete de celle d'une seule vidéo (ici on sauvegarde plusieurq vidéo [{vid1}{vid2}])
    def export_flagged_frames(self):
        if not self.key_frames:
            print("Aucun flag à exporter.")
            return
        date_str = datetime.now().strftime("%Y-%m-%d") 
        #regenerer un nouveau doissier pour ne pas rajouter à l'ancien
        output= "flags_dir_" + date_str 
        os.makedirs(output,exist_ok=True)
        
        export_data={} #initilaisation
        key = f"video_num_{self.index}"
        export_data[key] = {
        ...
        for frame_idx in sorted(self.key_frames):
            ...
            ret, frame = self.cap.read()
            if ret:
                filename = os.path.join(output,f"frame_{frame_idx:04d}.jpg")
                cv2.imwrite(filename, frame)
                print(f"Exporté : {filename}")
            else:
                print(f"Erreur lors de l'export de la frame {frame_idx}")
        json_path = os.path.join(output,f"flags_multiple _{date_str}.json")
        if os.path.exists(json_path):
            try:
                with open(json_path,"r") as f:
                    all_export = json.load(f)
            except json.JSONDecodeError:
                all_export =[]
        else:
            all_export =[]

        all_export.append(export_data)
        with open(json_path, "w") as f:
            json.dump(all_export, f, indent=4)

        print(f"\nexport terminé : {len(self.key_frames)} images et les données sauvegardées")
